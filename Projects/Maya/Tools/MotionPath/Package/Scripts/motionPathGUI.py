...
class MotionPathWidget(QtWidgets.QDockWidget):
    def __init__(self, parent=None):
        if cmds.window("Motion Path", exists=True):
            cmds.deleteUI("Motion Path")

        super(MotionPathWidget, self).__init__(parent)
        self.setWindowTitle("Motion Path")
        self.setObjectName(WCN)
...
def _get_dockable_motion_path_widget():
    ...
    for top in tops:
        if top.objectName() == WCN:
            top.close()
            break

    tc_motion_path_widget = MotionPathWidget(maya_main_window())
...
def run():
    if not cmds.pluginInfo('motionPath', q=1, l=1):
        cmds.loadPlugin('motionPath', quiet=True)

    # tc_motion_path_widget = MotionPathWidget(maya_main_window())
    # tc_motion_path_widget.show()
    # # tc_motion_path_widget.show(dockable=True, area='right', floating=False, retain=True)
